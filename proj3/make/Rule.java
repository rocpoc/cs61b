...
class Rule {
    ...
    void addCommands(List<String> commands) {
        if (!commands.isEmpty() && !_commands.isEmpty()) {
            throw new IllegalStateException("error non-empty");
        }
    ...
    private void checkFinishedDependencies() {
        for (int v : _depends.successors(this._vertex)) {
            if (_depends.getLabel(v).isUnfinished()) {
                throw new IllegalStateException("error successor not rebuilt");
            }
        }
    }
    ...
    private boolean outOfDate() {
        ...
        for (int v : _depends.successors(this._vertex)) {
            if (_depends.getLabel(v)._time > this._time) {
                return true;
            }
        }
        return false;
    ...
    void rebuild() {
        ...
        if (outOfDate()) {
            if (_commands.isEmpty()) {
                make.Main.error("%s error needs to be rebuilt, "
                                + "but has no commands",
                      _target);
            } else {
